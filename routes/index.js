...
router.get("/", (req, res) => {
	return NewsModel.find({}, (err, docs) => {
		...
		const content = { hupu: "", hupuOrder: 0, wangyi: "", wangyiOrder: 0, zhihu: "", zhihuOrder: 0 };
		...
		return res.type('.html').send(html.replace(/{HUPU}/, content.hupu).replace(/{WANGYI}/, content.wangyi).replace(/{ZHIHU}/, content.zhihu))
	});
});
